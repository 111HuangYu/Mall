...
function queryCategor() {
  $.ajax({
    type: "GET",
    url: "/category/list",//./ testjson / category.json
    data: "",
    dataType: "json",
...
function queryGoodsByCategoryId(cgId, cgName, pn, pz) {
  $.ajax({
    type: "GET",
    url: "/product/list",//./ testjson / goods.json
    data: {
      categoryId: cgId,
      pageNum: pn,
      pageSize: pz
...
function queryBanner() {
  $.ajax({
    type: "GET",
    url: "/banner/list",//./testjson/banner.json
    data: "",
    dataType: "json",
    success: function (res) {
      if (res.code == '200') {
  ...
  for (let good of goodsArr) {
    ...
    s +=
      `<div class="glid1">
          ...
          <div>
            <p class="gmoney"><i>￥</i>`+ good.price + `</p>
          </div>
        </div>
      </li>`
  }
