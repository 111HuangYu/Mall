...
var cartArr = {};//key是cart的id,值是商品的具体记录
...
function isLogin() {
  ...
  $.ajax({
    type: "GET",
    url: "/user/isLogin",//./testjson/isLogin.json
    data: {},
    async: false,//设置同步请求
    dataType: "json",
          ...
          s +=
            ...
            <ul class="cartul2" >
              ...
              <li class="cartli2">
                ...
                <div class="fl cartd5">` + record.prodName + `
                  <div class="cls"></div>
                </div>
              </li>
              ...
              <li class="cartli6"><a href="javascript:deleteCartGood(`+ record.id + `);">删除</a></li>
          </ul>
          </div>
          `
...
function updateCart(_id, _num) {
  $.ajax({
    type: "POST",
    url: "Cart/admin/update",//./testjson/updatecart.jsonv
    ...
    success: function (res) {
      if (res.code = 200) {
        $("#iid" + _id).val(_num);//界面更新了
        cartArr[_id].num = _num;//更新内存中对应商品的数量
        $("#gsum" + _id).html(_num * cartArr[_id].price);//更新该行的价格
...
function deleteCartGood(id) {
  ...
  $.ajax({
    type: "POST",
    url: "cart/admin/delete",//./testjson/deletecart.json
    contentType: "application/json",
    data: JSON.stringify(dataArr),
    dataType: "json",
    success: function (res) {
      if (res.code = 200) {
        delete cartArr[id];//删除内存中对应商品的数量
        //删除某个元素
        $("#cartgood" + id).remove();
        totalMoney();//更新总价
      } else {
